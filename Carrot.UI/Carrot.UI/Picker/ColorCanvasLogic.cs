...
namespace Carrot.UI.Controls.Picker {
    ...
    internal class ColorCanvasLogic {
        ...
        #endregion Alpha and Saturation things

        public ColorCanvasLogic(Canvas HostingCanvas) {
            this.HostingCanvas = HostingCanvas;
            Recreate_All();
        }

        public ColorCanvasLogic(Canvas HostingCanvas, Color ColorRGB) {
            this.HostingCanvas = HostingCanvas;
            Recreate_All();
            ColorARGB = ColorRGB;
        }
        ...
        private void Recreate_All() {
            ...
            #endregion Hexagon markers events
            ...
            #endregion Triangle markers events
        ...
        private void Redraw_TriangleMarkers() {
            ...
            double tmpPosition = (TriMarkTrans3.X - xPosition * 2);
        ...
        private bool PointInHexagon(Point sourcePoint) {
            ...
            if (sourcePoint.Y <= HexagonPointCenter.Y) {
                if (sourcePoint.X <= hexPnt3.X) {
                    if (PointInTriangle(sourcePoint, HexagonPointCenter, hexPnt3, hexPnt4))
                        return true;
                } else if (sourcePoint.X >= hexPnt2.X) {
                    if (PointInTriangle(sourcePoint, HexagonPointCenter, hexPnt1, hexPnt2))
                        return true;
                }
            } else {
                if (sourcePoint.X <= hexPnt5.X) {
                    if (PointInTriangle(sourcePoint, HexagonPointCenter, hexPnt4, hexPnt5))
                        return true;
                } else if (sourcePoint.X >= hexPnt6.X) {
                    if (PointInTriangle(sourcePoint, HexagonPointCenter, hexPnt6, hexPnt1))
                        return true;
                }
            }
            return false;
